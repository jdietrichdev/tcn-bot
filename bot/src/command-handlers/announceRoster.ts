...
export const handleAnnounceRoster = async (
  ...
  for (const rawRow of rosterData) {
    ...
    await sendMessage({ content: `<@&${config.clanRole}> below is the roster for the coming week, please get to where you need to be!` }, config.announcementChannel);
    for (const message of messages) {
        const { id } = await sendMessage({ content: message.content!.split("\n")[0]}, config.announcementChannel);
        await updateMessage(config.announcementChannel, id, message);
