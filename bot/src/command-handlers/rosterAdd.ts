...
export const handleRosterAdd = async (
  ...
  try {
    ...
    try {
      console.log(`Quick refresh: removing ${playerName} from cached unrostered lists`);
      const { refreshUnrosteredMessagesQuick } = await import('../component-handlers/unrosteredButton');
      await refreshUnrosteredMessagesQuick(playerName);
