...
export const handleTaskComplete = async (
  ...
  try {
    ...
    const responseData = await performTaskAction(interaction, taskId, guildId, 'complete', notes);
    ...
    console.log(`Slash command /task-complete processed for task ${taskId}`);

  } catch (err) {
    console.error('Failed to handle /task-complete command:', err);
    await updateResponse(interaction.application_id, interaction.token, {
